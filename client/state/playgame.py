...
class PlayGame(GameState):
    ...
    def __init__(self, window: pygame.Surface):
        ...
        self.grass_img = pygame.image.load(
            os.path.join(os.getcwd()) + "/client/assets/grass.jpg"
        )
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1:
                    ...
                    projectile = FlameShot(
                        self.main_player.x, self.main_player.y, direction
                    )
        ...
        for x in range(num_x):
            ...
            for y in range(num_y):
                self.window.blit(
                    self.grass_img,
                    (
                        start_x + x * c.grass_image_width,
                        start_y + y * c.grass_image_height,
                    ),
                )
